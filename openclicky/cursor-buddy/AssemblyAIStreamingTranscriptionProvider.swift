...
final class AssemblyAIStreamingTranscriptionProvider: BuddyTranscriptionProvider {
    ...
    /// This must be provided by the app bundle; the old placeholder URL should
    /// never be treated as a valid configuration.
    private static let placeholderTokenProxyURL = "https://your-worker-name.your-subdomain.workers.dev/transcribe-token"
    ...
    private var resolvedTokenProxyURL: URL? {
        guard let tokenProxyURLString else { return nil }
        guard tokenProxyURLString != Self.placeholderTokenProxyURL else { return nil }
        return URL(string: tokenProxyURLString)
    }

    /// Calls the Cloudflare Worker to get a short-lived AssemblyAI token.
    private func fetchTemporaryToken(from tokenProxyURL: URL) async throws -> String {
