    ...
    private func makeAPIRequest() throws -> URLRequest {
        guard let apiKey, !apiKey.isEmpty else {
            throw NSError(
                domain: "ClaudeAPI",
                code: -1000,
                userInfo: [NSLocalizedDescriptionKey: "Anthropic is not configured. Set AnthropicAPIKey in Info.plist or ANTHROPIC_API_KEY in the launch environment."]
            )
        }
    ...
    func analyzeImage(
        ...
        guard let httpResponse = response as? HTTPURLResponse,
            ...
            throw NSError(
                ...
                userInfo: [NSLocalizedDescriptionKey: "API Error (content-type: \(contentType)): \(responseString)"]
            )
        }
