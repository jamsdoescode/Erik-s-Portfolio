...
final class CompanionManager: ObservableObject {
    ...
    private static let doneAgentModeDefaultsKey = "isDoneAgentModeEnabled"

    static var isDoneAgentModeEnabled: Bool {
        UserDefaults.standard.object(forKey: doneAgentModeDefaultsKey) as? Bool ?? true
    }
    ...
    func showQuickTextInputFromMenuBar() {
        showNotchTextInput { [weak self] submittedText in
            guard let self else { return }
            if Self.isDoneAgentModeEnabled {
                DoneExecutor.submitIntent(submittedText, companionManager: self, source: "menu_bar_done")
            } else {
    ...
    private func showMainOpenClickyPanelFromShortcut() {
        ...
        notchCaptureWindowManager.showTextInput { [weak self] submittedText in
            guard let self else { return }
            if Self.isDoneAgentModeEnabled {
                DoneExecutor.submitIntent(submittedText, companionManager: self, source: "notch_shortcut_done")
            } else {
