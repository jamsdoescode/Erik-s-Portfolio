...
struct CompanionPanelView: View {
    ...
    @State private var isOrganizingDownloads: Bool = false
    @State private var doneAgentStatusMessage: String?
    @State private var isDoneAgentRunning: Bool = false
    ...
    var body: some View {
        ...
        .task {
            isDoneAgentRunning = await DoneAgentClient.isAgentRunning()
        }
    }
    ...
    // MARK: - Done quick actions

    private var doneQuickActionsSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 6) {
                ...
                Circle()
                    .fill(isDoneAgentRunning ? DS.Colors.success : DS.Colors.warning)
                    .frame(width: 6, height: 6)

                Text(isDoneAgentRunning ? "Agent on" : "Agent off")
                    .font(.system(size: 10, weight: .medium))
                    .foregroundColor(DS.Colors.textTertiary)
            }

            Text("Automate outcomes on your Mac. Double-tap Shift to type any task, or use Organize Downloads below.")
                .font(.system(size: 10, weight: .medium))
                .foregroundColor(DS.Colors.textTertiary)
                .fixedSize(horizontal: false, vertical: true)

            Button(action: {
                Task { await runOrganizeDownloads(dryRun: false) }
            }) {
                HStack(spacing: 8) {
                    if isOrganizingDownloads {
                        ProgressView()
                            .controlSize(.small)
                    ...
                    Text(isOrganizingDownloads ? "Organizing…" : "Organize Downloads")
                        .font(.system(size: 12, weight: .semibold))
                }
                .foregroundColor(DS.Colors.textOnAccent)
                .frame(maxWidth: .infinity)
            ...
            .buttonStyle(.plain)
            .pointerCursor()
            .disabled(isOrganizingDownloads || !isDoneAgentRunning)
            .opacity(isDoneAgentRunning ? 1 : 0.55)

            if !isDoneAgentRunning {
                Text("Start the agent: cd agent && npm run dev")
                    .font(.system(size: 9, weight: .medium))
                    .foregroundColor(DS.Colors.textTertiary)
            }

            if let doneAgentStatusMessage {
                Text(doneAgentStatusMessage)
                    .font(.system(size: 10, weight: .medium))
                    .foregroundColor(DS.Colors.textSecondary)
                    .fixedSize(horizontal: false, vertical: true)
            }
    ...
    @MainActor
    private func runOrganizeDownloads(dryRun: Bool) async {
        isOrganizingDownloads = true
        doneAgentStatusMessage = nil
        defer { isOrganizingDownloads = false }

        do {
            let result = try await DoneAgentClient.organizeDownloads(dryRun: dryRun)
            let summary = result.summary ?? "Downloads organized."
            let moveCount = result.movesApplied ?? 0
            doneAgentStatusMessage = "\(summary) (\(moveCount) moves)"

            _ = OpenClickyDesktopNotificationCenter.shared.post(
                title: "Done.",
                body: summary,
                threadID: "done.organize-downloads",
                userInfo: ["source": "done_agent"]
            )
        } catch {
            doneAgentStatusMessage = error.localizedDescription
        }
    }
