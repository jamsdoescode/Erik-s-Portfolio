...
### 1. Add files to the `cursor-buddy` target

In Xcode → File → Add Files to "cursor-buddy"… → select all eight new
...
Make sure **Add to targets: cursor-buddy** is ticked. The three existing-file
...
`0.4.0` → add the `GLTFSceneKit` library product to the `cursor-buddy`
target.
...
1. **UserDefaults** (production path): set `OpenClicky.Tripo3D.APIKey` from
   ...
   ```bash
   defaults write com.openclicky.cursor-buddy OpenClicky.Tripo3D.APIKey 'tsk_...'
   ```
